...
# 6 hour strategy re-selection
SELECTION_INTERVAL_MINUTES = 360

# Position monitoring
MONITOR_INTERVAL_SEC = 30

# Telegram position report
TELEGRAM_REPORT_INTERVAL_SEC = 300

# Maximum simultaneous selected symbols
MAX_SELECTIONS = 6

# 15% of balance as margin per trade
...
# Activate trailing after price moves 1%
...
# After target is realized:
# wait 10 minutes and automatically resume
TARGET_COOLDOWN_SEC = 10 * 60
...
def validate_config():
    ...
    if not TELEGRAM_API_ROOT:
        missing.append("TELEGRAM_API_ROOT")
    ...
    for coin in selected_coins:
        ...
        if not trailing_ok:
            ...
            send_telegram(
                ...
                f"`{activation_price}`\n"
                f"📉 Callback: "
                f"`{TRAILING_CALLBACK_RATE}%`\n"
                f"❌ Error:\n"
                f"`{str(trailing_order)[:1500]}`\n\n"
                f"🛡️ Emergency static SL "
                f"тавьж байна."
            )
            ...
            if is_api_error(sl_order):
                ...
                send_telegram(
                    ...
                    f"🛡️ *EMERGENCY SL ACTIVE*\n"
                    f"`{symbol}`\n"
                    f"SL: "
                    f"`{emergency_sl_price}`"
                )
...
def handle_target_reached(
    ...
    send_telegram(
        ...
        f"💰 Realized PnL "
        f"(trade history): "
        f"`{'+' if target_realized >= 0 else ''}"
        f"${target_realized:.2f}`\n"
        f"💵 Balance change: "
        f"`{'+' if balance_delta >= 0 else ''}"
        f"${balance_delta:.2f}`\n"
        f"💵 New Balance: "
        f"`${balance_after:,.2f}`\n"
        f"🔒 Open Positions: `0`\n\n"
        f"😴 *10 МИНУТ COOLDOWN*\n"
        f"🔄 Дараа нь автоматаар "
        f"дахин trading эхэлнэ."
    )
...
def target_cooldown():
    ...
    send_telegram(
        ...
        "━━━━━━━━━━━━━━━━━\n"
        "🤖 Бот дахин ажиллаж байна.\n"
        "🔍 Шинэ market screening эхэллээ."
    )
...
def monitor_positions():
    ...
    msg += (
        ...
        f"💰 Session Realized: "
        f"`{'+' if session_realized_pnl >= 0 else ''}"
        f"${session_realized_pnl:.2f}`"
    )
...
def main():
    ...
    send_telegram(
        ...
        "3️⃣ Verify position = 0\n"
        "4️⃣ Realized profit\n"
        "5️⃣ Wait 10 minutes\n"
        "6️⃣ Auto resume trading"
    )
